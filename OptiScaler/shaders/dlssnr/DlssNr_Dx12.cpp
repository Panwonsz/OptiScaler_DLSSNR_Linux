...
// DLSS5_NR_PIN=1 stops parked resources from ever being released. It leaks a handful of textures for
// the life of the process, which is a fine price for an answer: a GPU hang report has named a
// use-after-free of a destroyed committed texture, read ~310 ms after it was unbound, and the only
// thing this pass ever destroys under in-flight work is what the graveyard below releases after 32
// frames. If the hang goes away with this set, the 32-frame deferral is too short for an
// asynchronous model that takes 250 ms, and that is the bug.
bool PinNrResources()
{
    static int pin = -1;
    ...
    if (pin < 0)
    {
        ...
        if (GetEnvironmentVariableA("DLSS5_NR_PIN", value, sizeof(value)) != 0 && atoi(value) != 0)
            pin = 1;
    }
                              ...
                              ID3D12Resource** clone)
{
    ...
    if (*clone == nullptr)
    {
        *clone = CreateGuideClone(device, source);
        ...
        (*clone)->SetName(L"DLSS-NR guide clone");
                           ...
                           ID3D12CommandQueue* timingQueue)
{
    ...
    const auto workWidth = useHip ? DlssNr::Hip::ModelWidth : (unsigned int) (width * workScale + 0.5f);
    const auto workHeight = useHip ? DlssNr::Hip::ModelHeight : (unsigned int) (height * workScale + 0.5f);
    ...
    if (g_nr.output == nullptr)
    {
        ...
        // Named so that a GPU hang report can say which texture it was. Without this, vkd3d labels
        // them "Committed Texture (zeroed) (cookie 137)" and a use-after-free names nothing.
        if (g_nr.output != nullptr)
            g_nr.output->SetName(L"DLSS-NR output");
        if (g_nr.colorCopy != nullptr)
            g_nr.colorCopy->SetName(L"DLSS-NR colorCopy");
        if (g_nr.hdrCopy != nullptr)
            g_nr.hdrCopy->SetName(L"DLSS-NR hdrCopy");
    }
    ...
    const int result =
        useHip ? DlssNr::Hip::Evaluate(
               ...
               : g_nr.evaluate(cmdList, g_nr.feature, g_nr.capabilityParams, modelInput, depthIn, motionIn, g_nr.output,
